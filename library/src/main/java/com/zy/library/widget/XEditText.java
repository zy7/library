 ...
 * 修复EmojiInputFilter和LengthInputFilter冲突的问题
 * <p>
 * change by zhongyun on 2019-05-24.
 */
public class XEditText extends AppCompatEditText {
    ...
    private void setTextToSeparate(@NonNull CharSequence c, boolean fromUser) {
        ...
        if (fromUser) {
            ...
            if (mSelectionPos > getText().length()) {
                mSelectionPos = getText().length();
            }
            if (mSelectionPos < 0) {
                mSelectionPos = 0;
            }
    ...
    public void setDisableEmoji(boolean disableEmoji) {
        ...
        InputFilter[] oldFilter = getFilters();
        if(oldFilter != null && oldFilter.length > 0) {
            final int len = oldFilter.length;
            boolean hasEmojiFilter = false;
            ArrayList<InputFilter> newFilterList = new ArrayList<>(len + 1);

            for(int i=0; i<len; i++) {
                if(oldFilter[i] instanceof EmojiExcludeFilter) {
                    if(disableEmoji) {
                        hasEmojiFilter = true;
                        newFilterList.add(oldFilter[i]);
                    }
                } else {
                    newFilterList.add(oldFilter[i]);
                }
            ...
            if(disableEmoji && !hasEmojiFilter) {
                newFilterList.add(new EmojiExcludeFilter());
            }

            setFilters(newFilterList.toArray(oldFilter));
        } else {
            if (disableEmoji) {
